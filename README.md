...
> **Ethical Disclaimer:** All tests documented in this manual were performed on intentionally vulnerable or legally authorized targets only. Unauthorized scanning of systems is illegal. This manual is for **educational purposes only**.
...
The following features require paid third-party API credentials regardless of the target. These are permanent limitations of the tool itself, not scan results:
...
For Whois and port scanning, standalone tools like the `whois` command and direct `nmap` are recommended as free alternatives.
...
- Why some features require paid API credentials — a real-world tool limitation
